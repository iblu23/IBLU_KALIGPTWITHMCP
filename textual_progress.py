...
# Define classes that work both with and without Textual
...
# Textual-specific classes (only defined when Textual is available)
if TEXTUAL_AVAILABLE:
    ...
    class TextualProgressManager:
        ...
        def create_progress_session(self, title: str, tasks: List[Dict[str, Any]]) -> None:
            """Create a fallback progress session"""
            print(f"🎨 {title} (Fallback Mode)")
            for task in tasks:
                print(f"  📦 {task['name']}")
            return None
