...
print_header() {
    ...
    echo -e "${CYAN}║      ╩ ╩╩ ╩╚═╝╩ ╩   ╩ ╩ ╩╚═╝      ╚╩╝╚═╝╩╚═╩═╝═╩╝   🧠                     ║${NC}"
    echo -e "${CYAN}║                                                                              ║${NC}"
    echo -e "${CYAN}║        🔥 IBLU PROFESSIONAL HACKING ASSISTANT v2.3 🔥                        ║${NC}"
    echo -e "${CYAN}║        🐍 Advanced Python Cybersecurity Toolkit                              ║${NC}"
    echo -e "${CYAN}║                                                                              ║${NC}"
    echo -e "${CYAN}╠══════════════════════════════════════════════════════════════════════════════╣${NC}"
    echo -e "${CYAN}║  🧩 PLATFORM                                                                  ║${NC}"
    echo -e "${CYAN}║  • 🤖 Multiple AI providers (Perplexity, OpenAI, Gemini, Mistral)             ║${NC}"
    echo -e "${CYAN}║  • 🔗 HexStrike MCP integration (150+ security tools)                         ║${NC}"
    echo -e "${CYAN}║                                                                              ║${NC}"
    echo -e "${CYAN}║  🚀 CAPABILITIES                                                              ║${NC}"
    echo -e "${CYAN}║  • 🛡️ 122+ professional security commands                                     ║${NC}"
    echo -e "${CYAN}║  • 📡 Real-time vulnerability assessment                                      ║${NC}"
    echo -e "${CYAN}║  • 💣 Automated payload generation                                            ║${NC}"
    echo -e "${CYAN}║  • 🗺️ Network reconnaissance & mapping                                        ║${NC}"
    echo -e "${CYAN}║  • 📊 Reporting & documentation                                               ║${NC}"
    echo -e "${CYAN}║                                                                              ║${NC}"
    echo -e "${CYAN}║  ⚠️ AUTHORIZED USE ONLY — professional security testing platform              ║${NC}"
    echo -e "${CYAN}╚══════════════════════════════════════════════════════════════════════════════╝${NC}"
    echo
}
